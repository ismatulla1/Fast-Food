...
from database import (
    is_admin, is_new_foods, is_progress_foods, update_order,
    add_food, get_foods, delete_food, get_users, update_food_data
)

# Keyboard va matnlar
from .buttons import register_kb
from .admin_button import (
    admin_munu_text, admin_menu,
...
async def start_admin(message: Message):
    ...
    if user:
        ...
        await message.answer(
            "Siz Ro'yhatdan o'tmagansiz, Ro'yhatdan o'ting: ",
...
async def get_food_desc(message: Message, state: FSMContext):
    if len(message.text) < 500:
        ...
        await message.answer("Taom izohi juda uzun, qayta kiriting.")
...
async def get_food_image(message: Message, state: FSMContext):
    try:
        ...
        file_path = f"images/{food.get('name')}.jpg"
        ...
        await message.answer("Rasm yuklashda muammo bo‘ldi, qayta yuboring.")
...
async def get_food_price(message: Message, state: FSMContext):
    if message.text.isdigit():
        ...
        await message.answer("Taom miqdorini kiriting:")
    else:
...
async def get_food_quantity(message: Message, state: FSMContext):
    if message.text.isdigit():
        ...
        await state.clear()
        add_food(data)
        await message.answer("✅ Taom muvaffaqiyatli saqlandi.", reply_markup=menu_food_button)
    else:
